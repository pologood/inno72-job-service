...
import com.inno72.job.core.thread.ExecutorRegistryThread;
import com.inno72.job.core.thread.TriggerCallbackThread;
		...
		thread = new Thread(new Runnable() {
			...
			public void run() {
				...
				try {
					// Start server
					server.start();
					logger.info(">>>>>>>>>>> job jetty server start success at port:{}.", port);
					...
					logger.info(">>>>>>>>>>> rpc server join success, netcon={}, port={}", JettyServer.class.getName(), port);
				} catch (Exception e) {
